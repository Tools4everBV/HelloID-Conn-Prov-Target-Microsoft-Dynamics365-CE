#################################################
# HelloID-Conn-Prov-Target-Microsoft-Dynamics365-CE-Enable
# PowerShell V2
#################################################
...
try {
    ...
    switch ($lifecycleProcess) {
        'EnableAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Enabling Microsoft-Dynamics365-CE account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatEnableParams
            }
            else {
                Write-Information "[DryRun] Enable Microsoft-Dynamics365-CE account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not enable Microsoft-Dynamics365-CE account: [$($actionContext.References.Account)]. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditLogMessage = "Could not enable Microsoft-Dynamics365-CE account: [$($actionContext.References.Account)]. Error: $($_.Exception.Message)"
