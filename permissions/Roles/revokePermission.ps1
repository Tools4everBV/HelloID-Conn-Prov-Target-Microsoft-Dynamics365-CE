#################################################################
# HelloID-Conn-Prov-Target-Microsoft-Dynamics365-CE-RevokePermission-Roles
# PowerShell V2
#################################################################
...
try {
    ...
    switch ($lifecycleProcess) {
        'RevokePermission' {
            $splatRevokeRoleParams = @{
                Uri     = "$($actionContext.Configuration.BaseUrl)/api/data/v9.2/systemusers($($correlatedAccount.systemuserid))/systemuserroles_association($($actionContext.References.Permission.Reference))/`$ref" 
                Method  = 'Delete'
                Headers = $headers
            }

            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Revoking Microsoft-Dynamics365-CE permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"

                $null = Invoke-RestMethod @splatRevokeRoleParams
            }
            else {
                Write-Information "[DryRun] Revoke Microsoft-Dynamics365-CE permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditLogMessage = "Could not revoke Microsoft-Dynamics365-CE permission for account: [$($actionContext.References.Account)]. Error: $($errorObj.FriendlyMessage). Action initiated by: [$($actionContext.Origin)]"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    }
    else {
        $auditLogMessage = "Could not revoke Microsoft-Dynamics365-CE permission for account: [$($actionContext.References.Account)]. Error: $($_.Exception.Message). Action initiated by: [$($actionContext.Origin)]"
