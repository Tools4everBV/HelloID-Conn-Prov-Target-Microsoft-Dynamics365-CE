################################################################
# HelloID-Conn-Prov-Target-Microsoft-Dynamics365-CE-GrantPermission-Roles
# PowerShell V2
################################################################
...
try {
    ...
    switch ($lifecycleProcess) {
        'GrantPermission' {
            $splatGrantRoleParams = @{
                Uri     = "$($actionContext.Configuration.BaseUrl)/api/data/v9.2/systemusers($($correlatedAccount.systemuserid))/systemuserroles_association/`$ref"
                Method  = 'POST'
                Headers = $headers
                Body    = (@{
                    '@odata.id' = "$($actionContext.Configuration.BaseUrl)/api/data/v9.2/roles($($actionContext.References.Permission.Reference))"
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                ...
                $null = Invoke-RestMethod @splatGrantRoleParams
            }
            else {
