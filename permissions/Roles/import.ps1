####################################################################
# HelloID-Conn-Prov-Target-Microsoft-Dynamics365-CE-ImportPermissions-Roles
# PowerShell V2
####################################################################
...
try {
    ...
    $uri = "$($actionContext.Configuration.BaseUrl)/api/data/v9.2/systemuserrolescollection"

    do {
        $splatGetUserRolesParams = @{
            Uri     = $uri
            Method  = 'GET'
            Headers = $headers
        }
        $response = Invoke-RestMethod @splatGetUserRolesParams
        if ($response.value.Count -gt 0) {
            $groupedRoles = $response.value | Group-Object -Property 'roleid'
            foreach ($role in $groupedRoles) {

                $splatGetRoleParams = @{
                    Uri     = "$($actionContext.Configuration.BaseUrl)/api/data/v9.2/roles?`$filter=roleid eq '$($role.Name)'"
                    Method  = 'GET'
                    Headers = $headers
                }
                $roleDetails = Invoke-RestMethod @splatGetRoleParams

                if ($null -eq $roleDetails.value) {
                    throw "Could not retrieve role details for roleid: $($role.Name)"
                }

                $permission = @{
                    PermissionReference = @{
                        Reference = $roleDetails.value.roleid
                    }
                    Description         = "$($roleDetails.value.name)"
                    DisplayName         = "$($roleDetails.value.name)"
                ...
                $batchSize = 500
                $accountReferences = @($role.group.systemuserid) 
